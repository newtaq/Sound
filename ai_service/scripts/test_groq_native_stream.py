...
async def main() -> None:
    ...
    async for chunk in client.stream(
        text="Ответь ровно одной короткой фразой: Groq stream работает.",
        provider_name="groq",
        mode=AIMode.FAST,
        session_id="groq-native-stream-smoke-test",
        use_history=False,
        metadata={
            "script": "test_groq_native_stream",
        },
    ):
        event_counts[chunk.event_type.value] = event_counts.get(chunk.event_type.value, 0) + 1

        if chunk.metadata.get("stream_transport") == "groq_native_stream":
